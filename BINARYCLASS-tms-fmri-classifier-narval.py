...
# cohort labels
GROUPS    = ["Non-Trauma", "Trauma-Exposed"]

# 0 = Non-Trauma, 1 = Trauma-Exposed
LABEL_MAP = {
    "NTHC": 0, 
    "NTS": 0, 
    "TEHC": 1, 
...
def visualize_preprocessing(fs, pfx, subject_id, task, temp_dir, maps_dir):
    ...
    temp_nii.unlink(missing_ok=True) # Clean up the heavy raw file
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"vis_{subject_id}_{task}.png", dpi=300)
...
model = DenseNet121(
    ...
    out_channels=2        # 2 clinical groups (Non-Trauma, Trauma-Exposed)
).to(DEVICE)
